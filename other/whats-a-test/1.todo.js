...
const test = (foo, expected, message) => {
  const actual = foo()
  if (actual !== expected) {
    console.warn(`${message}: expected ${expected} but got ${actual} instead`)
  }
}
test(_ => sum(1, 2), 3, 'sum(1, 2)')
test(_ => subtract(3, 1), 2, 'subtract(3, 1)')
